...
def S_qf(r,delta,gamma,beta,k=1,qf=1):
    return qf*2*r*k*np.sin(delta-qf*(gamma-2*beta))/(1+r**2)

def A_qf(r,delta,gamma,beta,k=1,qf=1):
    return -2*r*k*np.cos(delta-qf*(gamma-2*beta))/(1+r**2)
...
def k_sinh(qt,qf,r,delta,gamma,beta,k,
           eff,d_eff,omega,d_omega,a_prod,a_det):
...
def k_sin(qt,qf,r,delta,gamma,beta,k,dm,sigma_t,
          eff,d_eff,omega,d_omega,a_prod,a_det):
...
def Amix_qf(t,qf,
            ...
            eff=1,d_eff=0,omega=0,d_omega=0,
            a_prod=0,a_det=0):
    first = P_t(t,qf,qf,dm,dg,gs,r,delta,gamma,beta,k,sigma_t,eff,d_eff,omega,d_omega,a_prod,a_det)
    second = P_t(t,-qf,qf,dm,dg,gs,r,delta,gamma,beta,k,sigma_t,eff,d_eff,omega,d_omega,a_prod,a_det)
    num = first - second
    den = first + second
    return num/den
