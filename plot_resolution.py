...
def plot_resolution(sigma_t,a_acc,n_acc,b_acc,beta_acc,cutoff_acc,
    ...
    B_f_obs_t = eff_acc*P_t(t=t,qt=1,qf=1,dm=dm,dg=dg,gs=gs,r=r,delta=delta_rad,gamma=gamma_rad,beta=beta_rad,sigma_t=sigma_t)
    Bbar_f_obs_t = eff_acc*P_t(t=t,qt=-1,qf=1,dm=dm,dg=dg,gs=gs,r=r,delta=delta_rad,gamma=gamma_rad,beta=beta_rad,sigma_t=sigma_t)
    B_fbar_obs_t = eff_acc*P_t(t=t,qt=1,qf=-1,dm=dm,dg=dg,gs=gs,r=r,delta=delta_rad,gamma=gamma_rad,beta=beta_rad,sigma_t=sigma_t)
    Bbar_fbar_obs_t = eff_acc*P_t(t=t,qt=-1,qf=-1,dm=dm,dg=dg,gs=gs,r=r,delta=delta_rad,gamma=gamma_rad,beta=beta_rad,sigma_t=sigma_t)
    # plotting options
    plot_B_f_t = B_f_obs_t if (qt_plus and qf_plus) else False
    plot_Bbar_f_t = Bbar_f_obs_t if (qt_min and qf_plus) else False
    plot_B_fbar_t = B_fbar_obs_t if (qt_plus and qf_min) else False
    plot_Bbar_fbar_t = Bbar_fbar_obs_t if (qt_min and qf_min) else False
    plot_osc(ax1,t,plot_B_f_t,plot_Bbar_f_t,plot_B_fbar_t,plot_Bbar_fbar_t,xmin,xmax,ymax=y_osc)
